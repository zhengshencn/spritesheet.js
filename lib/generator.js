...
exports.trimImages = function (files, options, callback) {
	...
	async.eachSeries(files, function (file, next) {
		...
		if (options.depth == null || parseInt(options.depth) == 8) {
			var depth = ' -depth 8 ';
		}
		else {
			var depth = ' -depth 16 ';
		...
		exec('convert' + scale + ' ' + fuzz + ' ' + depth + ' -define png:exclude-chunks=date "' + file.originalPath + '" -bordercolor transparent -border 1 -trim -set colorspace sRGB -colors 255 -type PaletteAlpha "' + file.path + '"', next);
	}, callback);
};
...
exports.generateImage = function (files, options, callback) {
	...
	var command = ['convert -define png:exclude-chunks=date -quality 0% ' + depth + ' -size ' + options.width + 'x' + options.height + ' xc:none'];
	var sourcePath = path.dirname(path.normalize(files[0].path));
	
	files.forEach(function (file) {
		command.push('"' + path.basename(file.path) + '" -geometry +' + (file.x + options.padding) + '+' + (file.y + options.padding) + ' -composite');
	});
	command.push(' -set colorspace sRGB -colors 255 -type PaletteAlpha "' + path.join(options.path, options.name) + '.png"');

	var cmdOptions = {"cwd": sourcePath};
	exec(command.join(' '), function (err) {
		if (err) return callback(err);

		unlinkTempFiles(files);
		exec('optipng "' + options.path + '/' + options.name + '.png"', function() {
			callback(null);
		});
