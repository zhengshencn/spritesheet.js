...
exports.generateImage = function (files, options, callback) {
	...
	var command = ['convert -define png:exclude-chunks=date -quality 0% -size ' + depth + ' ' + options.width + 'x' + options.height + ' xc:none'];
	...
	command.push(' -set colorspace sRGB -colors 255 -type PaletteAlpha "' + options.path + '/' + options.name + '.png"');
